...
cathe_path = os.path.join(os.curdir, "model_data")
if not os.path.exists(cathe_path):
    os.mkdir(cathe_path)
...
def train():
    ...
    torch.save(model.state_dict(), os.path.join(cathe_path, "trans_30.pt"))
...
def test():
    ...
    model = Transformer().to(device)
    model.load_state_dict(torch.load(os.path.join(cathe_path, "trans_30.pt")))
    model.eval()  # 训练时用.train()，预测时用.eval()
